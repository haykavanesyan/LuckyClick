...
async function endGame(room) {
    ...
    if ((greenCount + redCount) === 2) {
        ...
        notifyRoomPlayers(room, `[${room.id}] Вы не сделали ставку.`);
        return resetRoom(room);
    } else if (greenCount < redCount) { winners = room.green; winColor = 'Green'; }
...
💡 Как это работает:
➕ Пополнение: переведите TON на указанный кошелёк и укажите ваш Telegram ID в комментарии. После перевода через 2-3 минуты монеты зачислятся автоматически.
📤 Вывод: Следуйте указаниям Бота (Отправив сначала TON Адрес а потом Сумму которую хотите вывести). Заявка отправится администратору и будет обработана в течение 24 часов.
